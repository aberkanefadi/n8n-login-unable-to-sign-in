...
3- if settings says: `{"userActivated": false}` then do:
   ```sql
   UPDATE "user" SET "settings" = '{"userActivated": true}' WHERE "email" = 'your email';
   ```
...
```sql
UPDATE "user" SET "password" = '$2a$10$hash_goes_here' WHERE "email" = 'your email';
```

then exit the db using:
```sql
\q
```

and restart n8n. Problem should be fixed :)
